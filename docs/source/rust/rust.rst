...
我们都知道，C里面变量有两种内存模型，栈上的变量和堆上的变量
...
.. code-block:: c
	...
	{
		int a = 10; //a是栈上的内存
		int *b = malloc(sizeof(int));  // b是栈上的内存(指针) *b是堆上的内存
	}
	
	int main() {
		test();
		// a 和 b 的生命周期，在函数返回之后消亡， *b的内存依然存在
	}
...
上图完整的揭示了发生的过程
 ...
 - 第一：john 赋值发生的过程，同时申请了栈上的内存 以及堆上内存，并通过赋值建立关系
 - 第二、第三: Kent  赋值发生的过程,栈上的内存给了Kent,并且销毁了john变量
 - 第四、第五：函数退出，Kent生命周期结束，释放堆上内存，栈上内存随函数声明周期结束

为什么要这样实现？

 - 为了保证堆上内存和变量声明周期同时结束，堆上的内存只能属于一个人
...
上图完整的揭示了发生的过程
 ...
 - 第一：john 赋值发生,检查到类型提供了copy,则执行copy
 - 第二: Kent 函数退出，Kent/john生命周期结束，类型没有drop，栈上内存随函数声明周期结束
...
所有权转移:
	...
	RUST 变量赋值的过程，默认都会重新映射(建立绑定)，并且取消原有变量,我们把这个过程叫做所有权转移,实现了copy派生的类型，不会进行所有权转移，而是执行复制
...
引用也是一个基本类型(好比是指针)，该类型实现了copy，因此赋值没有发生所有权转移
...
转换
^^^^^^^
Result 提供了一个叫做OK的方法，可以把Result Ok(s) Err(e) 转为 Option类型 Some(s) None 
用来在错误处理之间转换

.. code-block:: c


	fn test(i:i32) -> Result<String, &'static str> {
		if i == 1 {
	...
	fn main() {
		let a = test(1).ok(); //test 返回的是result类型，但是经过OK方法，返回类型成为了option
		println!("{:?}",a);
	}
...
^^^^^^
 ...
 - Result 提供了OK 的方法，可以把Result 转换成Option
 - Result 提供了or 用于覆盖原有错误
...
面向对象
---------
之前，我们已经学习了RUST 的基本语法、所有权(所有权转移)规则  引用和引用使用规则，错误处理机制；其实已经能够像C一样完成基本的编程了，但是RUST不仅在安全性能上比C要考虑的多，更重要的是，RUST支持了面向对象的能力(实际上，RUST所有权转移这些能力也是基于面向对象实现的)，如果你学习或者了解过 JAVA/C++ 对面向对象一定不陌生了，让我们先从一个简单的示例讲起 
...
这是上一节的链表的一个代码片段，我们看到,可以通过 *impl Type*的语法，为某个结构体(类型) 实现类型函数,这没有什么好神奇的，其实就类似于C里面为某个结构体实现了几个函数，*&self* 告诉我们，结构体拥有的函数 可以通过这种方式，把自己传给函数，这也没有什么神奇的，下面是C语言的一个版本
...
.. code-block:: c
	...
	struct Node *new(struct Node *n,int value, struct Node *next) {
			n->value = value;
			n->next = next;
			return n;
	}
	...
	fn main() {
		struct Node n;
		struct Node *n_ptr = new(&n,10,NULL);
		show(n_ptr);
	}

trait初探
^^^^^^^^^^^
类似java 面向对象语言中，有继承和接口(也是一种继承)的概念，如果你不熟悉，也不要紧，让我们理解一下；

trait定义了特征组，该特征组里，定义了一些特征函数，结构体类型可以通过实现某个特征组，从而具备该特征组的能力，让我们先看一下一个简单的示例
...
.. code-block:: c
	...
	impl MyAdd for MyPair {
		fn add(&self, right: &Self) -> Self {
			MyPair{x: (self.x + right.x), y : (self.y + right.y)}
		}
 	}

	fn main(){
		let left = MyPair{x:10, y:20};
		let right = MyPair{x:10, y:20};
		let plus = left.add(&right);
		println!("x={} y={}", plus.x,plus.y);
		println!("x={} y={}", right.x,right.y);
	}

解释一下：

 - *trait MyAdd* 声明定义了一个特征集合 MyAdd,该特征集合里面目前只有一个特征方法: add 
 - trait里面可以使用 *self* 和 *Self* ，注意区分，一个是调用者实例变量的占位符，一个是实现trait的类型的占位符
 - 思考为什么使用了&Self, 而不是Self (巩固所有权转移)

trait可以解决什么问题？虽然我们这里还没有讲泛型，但是也可以先简单讨论一下
...
.. code-block:: c
	...
	impl MyAdd for MyPairInt {
		fn add(&self, right: &Self) -> MyPairInt {
			MyPairInt{x: (self.x + right.x), y : (self.y + right.y)}
		}
 	}
	
	impl MyAdd for MyPairFloat {
		fn add(&self, right: &Self) -> MyPairFloat {
			Self{x: (self.x + right.x), y : (self.y + right.y)}
		}
 	}

	fn main(){
		let left = MyPairInt{x:10, y:20};
		let right = MyPairInt{x:10, y:20};
		let plus = left.add(&right);
		println!("x={} y={}", plus.x,plus.y);
		println!("x={} y={}", right.x,right.y);
		
		let left = MyPairFloat{x:1.0, y:2.0};
		let right = MyPairFloat{x:1.0, y:2.0};
		let plus = left.add(&right);
		println!("x={} y={}", plus.x,plus.y);
		println!("x={} y={}", right.x,right.y);
	}

我们可以从上述代码，直观的感受到trait一个最重要的功能:  

 - 抽象了函数功能
 - 规定了特征函数的基本声明(参数、返回值)，任何实现该特征的结构体，都应该遵循他的函数定义

泛型初探
^^^^^^^^^^^
泛型为了解决什么问题？ ok，泛型存在的价值，其实就是为了解决代码冗余,还记得我们之前学习过的
错误返回机制吗? 无论是 Option/Result 提供的 *？* 操作符，也是为了解决 if/else这类冗余代码，让代码看的更加干净，但是实际上代码真正展开以后，依然是类似于if else的代码，看个示例
...
.. code-block:: c
	...
	fn main(){
		let i_max = max_i32(10,20);
		let f_max = max_f32(10.0,20.0);
		
		println!("{} {}",i_max, f_max);
	}

RUST和C语言，都是强类型语言，如果没有泛型的实现,必须要为不同类型实现不同的比较大小函数，看一下泛型的版本: 
...
.. code-block:: c
	...
	fn main(){
		let i_max = max(10,20);
		let f_max = max(10.0,20.0);
		
		println!("{} {}",i_max, f_max);
	}


暂时先不考虑 *std::cmp::PartialOrd* 这个标注，上述示例演示了如何让函数使用泛型 

 - 首先，在函数声明后面 通过 *<T>* 用来表示，该函数定义了一个类型占位符T 
 - 然后，由于之前已经声明过了类型占位符T，之后的函数实现就可以使用该类型 
...
.. code-block:: c
	...
	fn main()
	{
		let i_pair = MyPair::new(10,20);
		let f_pair = MyPair::new(10.0,20.0);
		
		println!("{}:{}  {}:{}",i_pair.x,i_pair.y, f_pair.x,f_pair.y);
	}
	
 - 首先，定义结构体的同时, 通过 *<T>* 声明该结构体可以使用泛型T，同时该结构体是一个泛型结构体
 - *impl<T>* 该声明表示当前的实现是一个泛型实现，使用了泛型T 
 - *MyPair<T>*  该声明表示当前实在为 泛型类型 MyPair 实现某个方法

trait bound
^^^^^^^^^^^^^^
trait bound,是为泛型增加约束的手段 比如下面这个示例 
...
.. code-block:: c
	...
	impl <T:Display> std::fmt::Display for MyPair<T> {
	    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
	...
	fn main() {
		let val = MyPair{x: MyType{val:10}, y: MyType{val:20}};
		println!("{}",val);
	}

我们希望为 MyPair<T> 实现默认打印输出的能力，由于内部实现有需要格式化输出 x y，因此必须要求
x,y有Display的实现 

 - <T: Display> 对泛型T增加了约束，要求泛型T 只能是实现了Display trait的类型
